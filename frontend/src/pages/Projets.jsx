...
const C={bg:'#F0F4F9',white:'#FFFFFF',navy:'#0A1628',blue:'#2563EB',teal:'#0D9488',green:'#16A34A',orange:'#EA580C',red:'#DC2626',gold:'#D97706',gray:'#6B7280',border:'rgba(10,15,30,.06)',shadow:'0 1px 3px rgba(10,15,30,.06),0 0 0 1px rgba(10,15,30,.06)',shadowMd:'0 4px 16px rgba(10,15,30,.08)'};
const card={background:C.white,borderRadius:14,boxShadow:C.shadow};
const TC=['#0D9488','#7C3AED','#1D4ED8','#EA580C','#D97706','#16A34A'];
const BASE='https://backend-one-kappa-96.vercel.app';
const tok=()=>localStorage.getItem('token')||sessionStorage.getItem('token')||'';
const hdr=()=>({'Authorization':'Bearer '+tok()});

function Spin(){return <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" style={{animation:'spin 1s linear infinite'}}><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>;}

// ── PhotoUpload: bouton natif CSS overlay ──────────────────────────────────
function PhotoUpload({phaseId,execPhaseId,isCat,photos=[],onDone}){
  ...
  const upload=async(files)=>{
    ...
    for(const f of Array.from(files)){
      ...
      try{
        ...
        const txt=await r.text();let d;try{d=JSON.parse(txt);}catch{throw new Error(txt.slice(0,100));}
        if(!r.ok)throw new Error(d.message||'HTTP '+r.status);
        added.push(d);
      }catch(e){setErr(e.message);}
    }
    setBusy(false);
    if(added.length){onDone([...photos,...added]);setMsg(added.length+' photo'+(added.length>1?'s':'')+' ajoutée'+(added.length>1?'s':''));}
  };

  const del=async(pid)=>{
    if(!window.confirm('Supprimer ?'))return;
    await fetch(BASE+(isCat?'/project-phase-photos/'+pid:'/project-execution-photos/'+pid),{method:'DELETE',headers:hdr()});
    onDone(photos.filter(p=>p.id!==pid));
  };
  ...
  return(
    <div onClick={e=>e.stopPropagation()} style={{marginTop:12}}>
      ...
      {/* Zone upload — input transparent par-dessus le visuel */}
      <div style={{position:'relative',borderRadius:9}}>
        <div style={{border:'1.5px dashed rgba(37,99,235,.4)',borderRadius:9,padding:'14px',textAlign:'center',background:'rgba(37,99,235,.02)',pointerEvents:'none'}}>
          {busy
            ?<div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,color:C.blue,fontSize:13,fontWeight:600}}><Spin/>Upload...</div>
            :<div>
      ...
      {msg&&!busy&&<div style={{marginTop:6,padding:'7px 10px',background:'#F0FDF4',borderRadius:7,color:C.green,fontSize:12}}>✓ {msg}</div>}
    </div>
  );
}
  ...
  return(
    <div>
      <div style={{marginBottom:24}}>
        ...
        <div style={{fontSize:13.5,color:C.gray}}>Référentiel opérationnel — IP Core, Rural Start et autres types</div>
      </div>
      {!types.length?<div style={{...card,textAlign:'center',padding:48,color:C.gray}}>Aucun type de projet configuré.</div>
      :<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))',gap:14}}>
        {types.map((t,i)=>{const col=t.color||TC[i%TC.length];return(
          <div key={t.id} onClick={()=>onSelect(t)}
            style={{...card,cursor:'pointer',borderTop:'3px solid '+col,paddingTop:16,padding:'16px 18px 18px',transition:'all .18s'}}
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
              <div style={{width:36,height:36,borderRadius:9,background:col+'18',display:'flex',alignItems:'center',justifyContent:'center',color:col,fontSize:16,fontWeight:700}}>{t.name?.charAt(0)||'P'}</div>
              <div><div style={{fontSize:15,fontWeight:700,color:C.navy}}>{t.name}</div><div style={{fontSize:11,color:C.gray}}>{t.category}</div></div>
            </div>
            <div style={{fontSize:13,color:C.gray,lineHeight:1.55,marginBottom:14}}>{t.description?.slice(0,90)}{t.description?.length>90?'...':''}</div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <span style={{fontSize:11.5,fontWeight:600,color:col,background:col+'15',padding:'3px 10px',borderRadius:20}}>{t.phases_count||'—'} phases</span>
  ...
  return(
    <div>
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20}}>
        <button onClick={onBack} style={{padding:'6px 14px',borderRadius:8,border:'1px solid '+C.border,background:C.white,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:600,color:C.navy}}>← Catalogue</button>
        <div style={{flex:1}}>
            ...
            {sites.map(s=>{const pct=s.total_phases?Math.round(((s.done_phases||0)/s.total_phases)*100):0;return(
              <div key={s.id} style={{...card,cursor:'pointer',transition:'all .18s',padding:'16px 18px'}}
                ...
                <div style={{display:'flex',justifyContent:'space-between',marginBottom:10}}>
                  <div><div style={{fontSize:14.5,fontWeight:700,color:C.navy}}>{s.site_code}</div><div style={{fontSize:12,color:C.gray,marginTop:2}}>{s.site_name||s.region}</div></div>
...
function AttachSiteForm({type,onBack,onSuccess}){
  ...
  const submit=async()=>{
    ...
    try{await api.post('/project-catalogue/'+type.id+'/sites',form);onSuccess();}
    catch(e){setError(e.response?.data?.message||'Erreur');setSaving(false);}
  };

  return(
    <div>
      <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:24}}>
        <button onClick={onBack} style={{padding:'6px 14px',borderRadius:8,border:'1px solid '+C.border,background:C.white,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:600,color:C.navy}}>← Retour</button>
        <div><div style={{fontSize:18,fontWeight:700,color:C.navy}}>Rattacher un site à {type.name}</div><div style={{fontSize:12.5,color:C.gray,marginTop:2}}>Les {type.phases_count||'—'} phases seront copiées automatiquement</div></div>
      </div>
      <div style={{...card,maxWidth:500,padding:'22px 24px'}}>
        {[{l:'Code site (DUID)',f:'site_code',type:'select'},{l:'Nom du site',f:'site_name',ph:'Tour MTN Garoua Centre'},{l:'Client',f:'client',ph:'MTN Cameroun'},{l:'Référence BC',f:'bc_reference',ph:'416121376123-2'},{l:'Région',f:'region',ph:'Nord Cameroun'},{l:'Date démarrage',f:'start_date',it:'date'}].map(({l,f,ph,type:ft,it})=>(
          <div key={f} style={{marginBottom:14}}>
            <label style={{display:'block',fontSize:12,fontWeight:600,color:C.gray,marginBottom:5,textTransform:'uppercase',letterSpacing:'.04em'}}>{l}</label>
            {ft==='select'
              ?<select value={form[f]} onChange={e=>{const s=sites.find(x=>x.code===e.target.value);if(s)setForm(p=>({...p,site_code:s.code,site_name:s.name,region:s.region||''}));else setForm(p=>({...p,site_code:e.target.value}));}} style={{...inp,appearance:'none'}}>
                <option value="">Saisir ou sélectionner</option>
                {sites.map(s=><option key={s.id} value={s.code}>{s.code} — {s.name}</option>)}
              </select>
              :<input type={it||'text'} value={form[f]} placeholder={ph} onChange={e=>setForm(p=>({...p,[f]:e.target.value}))} style={inp}/>
            }
          </div>
        ))}
        {error&&<div style={{padding:'8px 12px',background:'#FEF2F2',borderRadius:8,color:C.red,fontSize:13,marginBottom:14}}>{error}</div>}
        <button onClick={submit} disabled={saving} style={{width:'100%',padding:12,borderRadius:8,border:'none',background:C.navy,color:'white',cursor:saving?'wait':'pointer',fontFamily:'inherit',fontSize:14,fontWeight:600}}>
          {saving?'Rattachement...':'✓ Rattacher et copier les phases'}
        </button>
      </div>
    </div>
  );
...
function SiteExecution({execId,onBack}){
  ...
  const SV={pending:{bg:'#F1F5F9',text:'#64748B',l:'En attente'},in_progress:{bg:'#EFF6FF',text:C.blue,l:'En cours'},done:{bg:'#F0FDF4',text:C.green,l:'Terminé'},blocked:{bg:'#FEF2F2',text:C.red,l:'Bloqué'}};

  const load=useCallback(async()=>{
    try{const r=await api.get('/project-site-executions/'+execId);setExec(r.data);setPhases(r.data.phases||[]);const n={};(r.data.phases||[]).forEach(p=>{n[p.id]=p.notes||'';});setNotes(n);}
  ...
  const setStatus=async(pid,status)=>{
    try{await api.put('/project-execution-phases/'+pid,{status});setPhases(p=>p.map(x=>x.id===pid?{...x,status,started_at:status==='in_progress'&&!x.started_at?new Date().toISOString():x.started_at,completed_at:status==='done'?new Date().toISOString():null}:x));}
    catch(e){alert('Erreur statut');}
  };

  const saveNote=async(pid)=>{setSavingNote(pid);try{await api.put('/project-execution-phases/'+pid,{notes:notes[pid]});}catch(e){alert('Erreur note');}setSavingNote(null);};
  ...
  return(
    <div>
      <div style={{display:'flex',alignItems:'flex-start',gap:12,marginBottom:20}}>
        <button onClick={onBack} style={{padding:'6px 14px',borderRadius:8,border:'1px solid '+C.border,background:C.white,cursor:'pointer',fontFamily:'inherit',fontSize:13,fontWeight:600,color:C.navy,marginTop:2}}>← Retour</button>
        <div style={{flex:1}}>
        ...
        {phases.map((ph,idx)=>{
          ...
          return(
            <div key={ph.id} style={{...card,padding:0,overflow:'hidden',borderLeft:'3px solid '+(ph.status==='done'?C.green:ph.status==='in_progress'?C.blue:ph.is_client_scope?C.gold:C.border)}}>
              <div onClick={()=>setExpanded(isOpen?null:ph.id)} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'13px 16px',cursor:'pointer'}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{display:'flex',alignItems:'center',gap:7,flexWrap:'wrap'}}>
                    ...
                    <span style={{fontSize:10.5,fontWeight:600,padding:'2px 8px',borderRadius:20,background:st.bg,color:st.text}}>{st.l}</span>
                ...
                <div style={{padding:'0 16px 16px',borderTop:'1px solid '+C.border}}>
                  {!ph.is_client_scope&&(
                    <div style={{marginTop:14,marginBottom:14}}>
                      <div style={{fontSize:11,fontWeight:600,color:C.gray,marginBottom:8,textTransform:'uppercase',letterSpacing:'.05em'}}>Statut</div>
                      <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                        {[{s:'pending',l:'En attente',bg:'#F1F5F9',c:C.gray},{s:'in_progress',l:'En cours',bg:'#EFF6FF',c:C.blue},{s:'done',l:'Terminé',bg:'#F0FDF4',c:C.green},{s:'blocked',l:'Bloqué',bg:'#FEF2F2',c:C.red}].map(({s,l,bg,c})=>(
                          <button key={s} onClick={()=>setStatus(ph.id,s)} style={{padding:'6px 14px',borderRadius:20,border:'none',cursor:'pointer',fontFamily:'inherit',fontSize:12.5,fontWeight:600,background:ph.status===s?c:bg,color:ph.status===s?'white':c,transition:'all .15s'}}>{l}</button>
                        ))}
                      </div>
                    </div>
                  )}
                  <div style={{marginBottom:14}}>
                    ...
                    <textarea value={notes[ph.id]||''} onChange={e=>setNotes(n=>({...n,[ph.id]:e.target.value}))} placeholder="Observations..." rows={3}
                    ...
                    <button onClick={()=>saveNote(ph.id)} disabled={savingNote===ph.id} style={{marginTop:8,padding:'6px 14px',borderRadius:7,border:'none',background:C.navy,color:'white',cursor:'pointer',fontFamily:'inherit',fontSize:12,fontWeight:600}}>
                      {savingNote===ph.id?'Sauvegarde...':'💾 Sauvegarder'}
                    </button>
                  </div>
                  <div>
                    <div style={{fontSize:11,fontWeight:600,color:C.gray,marginBottom:6,textTransform:'uppercase',letterSpacing:'.05em'}}>Photos terrain ({ph.photos?.length||0})</div>
                    <PhotoUpload execPhaseId={ph.id} isCat={false} photos={ph.photos||[]} onDone={np=>updPh(ph.id,np)}/>
                  </div>
                </div>
              )}
            </div>
