...
`<p>Douala, le ${today}</p><p>Objet : ______________________________</p><br/><p>Madame, Monsieur,</p><br/><p>_____________________________________________</p><br/><p>Veuillez agréer nos cordiales salutations.</p><br/><p>La Direction</p>`}
  ...
  // Appelle Gemini avec les mêmes outils. Retourne un objet normalisé { content, tool_calls } comme Groq/OpenAI.
  const callGemini = async (sysPrompt, msgs, tools) => {
    if (!GEMINI_KEY) throw new Error('Clé Gemini non configurée');
    const body = {
    ...
    if (!res.ok) { const errData = await res.json().catch(()=>({})); throw new Error(`Gemini ${res.status}: ${errData.error?.message||'erreur inconnue'}`); }
  ...
  const send = useCallback(async(txt)=>{
    ...
    try {
      ...
      // ===== GROQ TOOL USE (avec fallback Gemini si Groq échoue) =====
