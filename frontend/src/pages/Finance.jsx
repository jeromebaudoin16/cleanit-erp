...
function TabEncaissements(){
  const clients=[
    {name:'MTN Cameroun',factures:12,montant:145000000,recu:98000000,retard:47000000,delai:35},
    {name:'Orange Cameroun',factures:5,montant:62000000,recu:62000000,retard:0,delai:28},
    {name:'CAMTEL',factures:3,montant:38000000,recu:12000000,retard:26000000,delai:67},
  ];
  return (
    <div style={{padding:'14px 20px',display:'flex',flexDirection:'column',gap:14}}>
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
        {[['245M FCFA','Total facturé clients',C.blue],['172M FCFA','Encaissé',C.green],['73M FCFA','En retard de paiement',C.red]].map(([v,l,c])=>(
      ...
      {clients.map(cl=>(
        <div key={cl.name} style={{background:C.white,border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden'}}>
          <div style={{padding:'12px 16px',background:C.bg,borderBottom:`1px solid ${C.border2}`,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
            <div style={{fontSize:13,fontWeight:600}}>{cl.name}</div>
            <div style={{display:'flex',gap:14,fontSize:12}}>
              <div style={{textAlign:'right'}}><div style={{fontSize:10,color:C.text3}}>Facturé</div><div style={{fontWeight:600}}>{(cl.montant/1000000).toFixed(0)}M</div></div>
              <div style={{textAlign:'right'}}><div style={{fontSize:10,color:C.text3}}>Reçu</div><div style={{fontWeight:600,color:C.green}}>{(cl.recu/1000000).toFixed(0)}M</div></div>
              {cl.retard>0&&<div style={{textAlign:'right'}}><div style={{fontSize:10,color:C.text3}}>Retard</div><div style={{fontWeight:600,color:C.red}}>{(cl.retard/1000000).toFixed(0)}M</div></div>}
              <div style={{textAlign:'right'}}><div style={{fontSize:10,color:C.text3}}>Délai moy.</div><div style={{fontWeight:600,color:cl.delai>45?C.red:cl.delai>30?C.orange:C.green}}>{cl.delai}j</div></div>
            </div>
          </div>
          <div style={{padding:'8px 16px'}}>
            <div style={{height:8,borderRadius:4,background:C.border2,overflow:'hidden'}}>
              <div style={{height:'100%',width:`${Math.round(cl.recu/cl.montant*100)}%`,background:C.green,borderRadius:4}}/>
            </div>
            <div style={{fontSize:10,color:C.text3,marginTop:4}}>{Math.round(cl.recu/cl.montant*100)}% encaissé — {cl.factures} facture{cl.factures>1?'s':''}</div>
          </div>
        </div>
      ))}
    </div>
