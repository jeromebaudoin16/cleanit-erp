...
// ── ICÔNES (style ligne, cohérent avec le reste de l'app) ─────────────────
...
const ICONS = {
  ...
  rack:      ['M5 3h14v18H5z','M5 8h14M5 13h14M5 18h14'],
};
...
const ITEMS = [
  ...
  { id:'cloudengine-dc', cat:'entreprise', icon:'router', name:'CloudEngine CE-Series (Data Center)', sub:'Ex. CE16800, CE9800, CE8800, CE6800',
    ...
    specs:["CE16800 : cœur de très grande capacité","CE9800/CE8800 : agrégation/cœur data center","CE6800 : accès/agrégation, le plus courant"] },
  { id:'netengine-ar', cat:'entreprise', icon:'router', name:'NetEngine AR (Routeur de succursale)', sub:'Ex. AR6700, AR6100, AR650, AR630, AR610',
  ...
  { id:'etp-cabinet', cat:'energie', icon:'battery', name:"Armoire ETP (Embedded Telecom Power)", sub:"Système d'alimentation site (ex. ETP48150/ETP48200)",
  ...
  { id:'r4850', cat:'energie', icon:'bolt', name:'Module redresseur R4850', sub:'Module de redressement enfichable',
    desc:"Module enfichable (\"hot-swap\") qui fait la conversion AC→DC à l'intérieur de l'armoire ETP ; plusieurs modules en parallèle selon la puissance totale du site.",
    usage:"S'insère dans les emplacements prévus de l'armoire ETP — remplaçable à chaud sans couper l'alimentation du site en cas de panne d'un module.",
  ...
  { id:'batterie-site', cat:'energie', icon:'battery', name:'Batteries de site (plomb ou lithium)', sub:'Autonomie de secours en cas de coupure secteur',
    ...
    usage:"Installées en armoire dédiée ou dans l'armoire ETP elle-même, raccordées au bus -48V ; l'autonomie dépend du nombre de strings installées et de la consommation du site.",
  ...
  { id:'smu', cat:'energie', icon:'router', name:'Unité de supervision (SMU/PMU)', sub:"Monitoring de l'alimentation et de l'environnement du site",
    ...
    usage:"Intégrée dans l'armoire ETP, raccordée aux capteurs du site (température, fumée, porte) ; transmet les alertes au centre de supervision via le réseau.",
  ...
  { id:'odf', cat:'fibre', icon:'fiber', name:'ODF (Optical Distribution Frame)', sub:'Tiroir de brassage / répartiteur optique',
  ...
  { id:'splice-closure', cat:'fibre', icon:'splice', name:"Boîte d'épissure / manchon fibre", sub:'Protection des soudures de fibre optique',
  ...
  { id:'connecteurs-fibre', cat:'fibre', icon:'splice', name:'Connecteurs & jarretières fibre (SC/LC/FC/ST)', sub:'Raccordement entre équipements et ODF',
...
export default function CatalogueOEM(){
  const [search, setSearch] = useState('');
  const [filter, setFilter] = useState('tous');
  const [selected, setSelected] = useState(null);

  const filtered = ITEMS.filter(it=>{
    const mc = filter==='tous' || it.cat===filter;
    const mq = !search || it.name.toLowerCase().includes(search.toLowerCase()) || it.sub.toLowerCase().includes(search.toLowerCase());
    return mc && mq;
  });

  return (
    <div style={{padding:'24px 28px',background:'#f5f7fa',minHeight:'100%'}}>
      <div style={{marginBottom:18}}>
        <h1 style={{fontSize:20,fontWeight:800,color:'#1e293b',margin:0}}>Catalogue Huawei</h1>
        <div style={{fontSize:12,color:'#64748b',marginTop:2}}>Référence du matériel et équipement utilisé sur les projets CleanIT — description et usage terrain</div>
      </div>
      ...
      <div style={{background:'white',borderRadius:10,padding:'12px 16px',border:'1px solid #e2e8f0',marginBottom:16,display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
        <div style={{position:'relative',flex:1,minWidth:200}}>
          ...
          <input placeholder="Rechercher (BBU, RTN, ODF, batterie...)" value={search} onChange={e=>setSearch(e.target.value)} style={{width:'100%',padding:'8px 12px 8px 32px',border:'1px solid #e2e8f0',borderRadius:8,fontSize:13,outline:'none',boxSizing:'border-box'}} />
        </div>
        <select value={filter} onChange={e=>setFilter(e.target.value)} style={{padding:'8px 12px',border:'1px solid #e2e8f0',borderRadius:8,fontSize:13,background:'white'}}>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:16}}>
        {CATEGORIES.map(c=>{
          const count = ITEMS.filter(it=>it.cat===c.id).length;
          return <span key={c.id} style={{padding:'4px 10px',borderRadius:12,fontSize:11,fontWeight:600,background:c.color+'15',color:c.color,border:`1px solid ${c.color}30`}}>{c.label} · {count}</span>;
        })}
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(260px,1fr))',gap:14}}>
        {filtered.map(it=>{
          ...
          return (
            <div key={it.id} onClick={()=>setSelected(it)} style={{background:'white',borderRadius:10,border:'1px solid #e2e8f0',padding:16,cursor:'pointer',transition:'all .15s'}}
        ...
        {!filtered.length && (
          <div style={{gridColumn:'1/-1',padding:30,textAlign:'center',color:'#64748b',fontSize:13}}>Aucun équipement trouvé</div>
        )}
      </div>

      {selected && (() => { const cat = CATEGORIES.find(c=>c.id===selected.cat); return (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',padding:20}} onClick={()=>setSelected(null)}>
          <div style={{background:'white',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'85vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,0.25)'}} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{padding:22}}>
              <p style={{fontSize:13,color:'#374151',lineHeight:1.7,margin:'0 0 14px'}}>{selected.desc}</p>
              ...
              <p style={{fontSize:13,color:'#374151',lineHeight:1.7,margin:'0 0 14px'}}>{selected.usage}</p>
