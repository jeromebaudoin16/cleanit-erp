...
export default function Inventaire() {
  ...
  const filtered = CATALOG_ITEMS.filter(it => {
  ...
  return (
    <div style={{padding:24,background:'#f0f2f5',minHeight:'100%'}}>
      <div style={{marginBottom:20}}>
        <h1 style={{fontSize:22,fontWeight:800,color:'#1e293b',margin:0}}>Inventaire OEM</h1>
        <p style={{color:'#64748b',fontSize:13,margin:'4px 0 0'}}>Catalogue de référence du matériel et équipement utilisé sur les projets — {CATALOG_ITEMS.length} équipements</p>
      </div>
      ...
      <div style={{background:'white',borderRadius:10,padding:'12px 16px',border:'1px solid #e2e8f0',marginBottom:16,display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
        <div style={{position:'relative',flex:1,minWidth:200}}>
          ...
          <input placeholder="Rechercher un équipement (BBU, RTN, OLT...)" value={search} onChange={e=>setSearch(e.target.value)} style={{width:'100%',padding:'8px 12px 8px 32px',border:'1px solid #e2e8f0',borderRadius:8,fontSize:13,outline:'none',boxSizing:'border-box'}} />
        </div>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:16}}>
        {CATALOG_CATEGORIES.map(c=>{
          const count = CATALOG_ITEMS.filter(it=>it.cat===c.id).length;
      ...
      {selected && (() => { const cat = CATALOG_CATEGORIES.find(c=>c.id===selected.cat); return (
          ...
          <div style={{background:'white',borderRadius:14,width:'100%',maxWidth:520,maxHeight:'85vh',overflowY:'auto',boxShadow:'0 24px 64px rgba(0,0,0,0.25)'}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:'18px 20px',background:cat.color,color:'white',display:'flex',justifyContent:'space-between',alignItems:'flex-start',position:'sticky',top:0}}>
              <div style={{display:'flex',gap:12,alignItems:'flex-start'}}>
                ...
                <div style={{width:38,height:38,borderRadius:8,background:'rgba(255,255,255,0.18)',display:selected.img?'none':'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}><CatIc d={CAT_ICONS[selected.icon]} size={19} color="white"/></div>
