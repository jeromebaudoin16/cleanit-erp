          ...
          {NAV.map(section => (
            <div key={section.section}>
              ...
              {section.items.map(item => {
                ...
                return (
                  <button key={item.path} onClick={() => nav(item.path)} title={!hovered ? t(item.label) : ''}
                    style={{
                    ...
                    {hovered && <span style={{overflow:'hidden', textOverflow:'ellipsis'}}>{t(item.label)}</span>}
                  </button>
                );
              })}
            </div>
              ...
              <div style={{position:'absolute', top:'calc(100% + 8px)', right:0, width:220, background:'white', borderRadius:12, boxShadow:'0 8px 32px rgba(0,0,0,0.12)', border:'1px solid #e8ecf0', zIndex:9999, overflow:'hidden'}}>
                ...
                <div style={{padding:'6px'}}>
                  {[
                    ...
                    <button key={t(item.label)} onClick={() => { setShowProfile(false); nav(item.path); }} style={{width:'100%', padding:'9px 12px', border:'none', background:'transparent', cursor:'pointer', display:'flex', alignItems:'center', gap:10, fontSize:13, color:'#374151', borderRadius:8, transition:'all .15s'}}
                      onMouseEnter={e => e.currentTarget.style.background='#f8fafc'}
                      onMouseLeave={e => e.currentTarget.style.background='transparent'}>
