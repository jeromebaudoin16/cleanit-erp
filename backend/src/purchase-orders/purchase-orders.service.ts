...
export class PurchaseOrdersService {
  ...
  async importPO(file?: Express.Multer.File) {
    let items: POItem[] = [];
  ...
  private detectModule(desc: string): string {
    const d = desc.toLowerCase();
    if (d.includes('bbu') || d.includes('rru') || d.includes('aau') || d.includes('routeur') || d.includes('antenne') || d.includes('hardware')) return 'Inventaire';
    if (d.includes('install') || d.includes('survey') || d.includes('swap') || d.includes('démant') || d.includes('maintenance')) return 'Planning';
    if (d.includes('formation') || d.includes('certif')) return 'RH';
    if (d.includes('contrat') || d.includes('sla')) return 'Contrats';
    return 'Inventaire';
  }

  private getDefaultItems(): POItem[] {
    return [
  ...
  private generateActions(items: POItem[]): POAction[] {
  ...
  async generateTracker(id: number): Promise<Buffer> {
    const po = await this.findOne(id);
    const workbook = new ExcelJS.Workbook();
    workbook.creator = 'CleanIT ERP';
    workbook.created = new Date();

    const headerFill: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: '0078D4' } };
    ...
    const items: POItem[] = (po?.items || []) as POItem[];
    items.forEach((item, idx) => {
      ...
      if (idx % 2 === 0) row.eachCell(cell => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F8FAFC' } }; });
      row.height = 20;
    });
    ...
    const actions: POAction[] = (po?.actions || []) as POAction[];
    actions.forEach((action, idx) => {
      ...
      if (idx % 2 === 0) row.eachCell(cell => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F5F3FF' } }; });
      row.height = 22;
    });
    ...
    planningItems.forEach((item, idx) => {
      ...
      row.eachCell(cell => { cell.border = thinBorder; cell.alignment = { vertical: 'middle' }; });
      if (idx % 2 === 0) row.eachCell(cell => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7ED' } }; });
      row.height = 20;
    });
    ...
    finData.forEach((row, idx) => {
      ...
      if (idx === 0) {
        ...
        if (idx % 2 === 0) r.eachCell(cell => { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F0FDF4' } }; });
